...
class ET_Client(object):
    ...
    def __init__(self, get_server_wsdl = False, debug = False, params = None, tokenResponse=None):
        ...
        if config.has_section(HTTP_PROXY_SECTION) and distutils.util.strtobool(
                config.get(HTTP_PROXY_SECTION, 'enabled')):
            self.http_proxy_settings['verify_ssl'] = True if distutils.util.strtobool(
                config.get(HTTP_PROXY_SECTION, 'verify_ssl')) else False
            self.http_proxy_settings['proxies'] = {
                'http': config.get(HTTP_PROXY_SECTION, 'http_url'),
                'https': config.get(HTTP_PROXY_SECTION, 'https_url')
            }
        if config.has_section(SOAP_PROXY_SECTION) and distutils.util.strtobool(
                config.get(SOAP_PROXY_SECTION, 'enabled')):
            self.soap_proxy_settings['verify_ssl'] = True if distutils.util.strtobool(
                config.get(SOAP_PROXY_SECTION, 'verify_ssl')) else False
